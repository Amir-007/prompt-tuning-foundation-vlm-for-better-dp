...
1. **Removed Gemini API Dependency**:
   - The previous `v10` implementation relied on Google's Gemini API which frequently encountered rate limits and required manual API key setups. 
...
2. **Default Local LLM Engines (No UI Prompts)**:
   ...
   - It runs completely locally within your Colab GPU instance. TinyLlama sits at ~1 GB footprint, sharing VRAM flawlessly alongside models like `ViT-B/16` CLIP.
...
3. **Standardized Dataset Loading** *(fixed in v11)*:
   ...
   - Replaced redundant directory traversal logic with elegant and centralized dataset dictionaries. Handles `.zip` extractions, Google Drive nested variants, and custom JSON splits smoothly.
   ...
   - **`full_dataset` fix**: `full_dataset` is now built as `ConcatDataset([train_dataset, val_dataset, test_dataset])` (all images), rather than incorrectly aliasing it to `test_dataset` alone. This is used for the embedding-space visualisations (PCA / t-SNE / UMAP) so they cover the full data distribution.
...
To run the unified `GenericJSONSplitDataset` functionality correctly, make sure your Google Drive environment contains the required structural shortcuts initialized according to notebook Step 3 (*AML Dataset / split_zhou_X.json* data mappings).
